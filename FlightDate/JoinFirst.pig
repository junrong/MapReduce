-- register and using the CSVLoader()
...
FilterJointFlights = FILTER JoinFlights BY arrTime < DepTime
						AND (ToDate(FirstDate, 'yyyy-MM-dd')>ToDate('2007-05-31','yyyy-MM-dd'))
						AND (ToDate(FirstDate, 'yyyy-MM-dd')<ToDate('2008-06-01','yyyy-MM-dd'))
						AND (ToDate(SecondDate, 'yyyy-MM-dd')>ToDate('2007-05-31','yyyy-MM-dd'))
						AND (ToDate(SecondDate, 'yyyy-MM-dd')<ToDate('2008-06-01','yyyy-MM-dd'));
...
STORE AvgDelay INTO 's3://cs6240northvirginia/output/JoinFirst_Output';
